...
pub const Bin = struct {
    ...
    pub fn getNumChildren(self: Bin) u32 {
        return @intCast(c.GST_BIN_NUMCHILDREN(self.ptr));
    }
    ...
    pub fn fromDescription(bin_description: [*:0]const u8, ghost_unlinked_pads: bool) !Bin {
        ...
        if (element_ptr == null) {
            if (err) |e| {
                std.log.err("Failed to parse bin: {s}", .{e.message});
                c.g_error_free(e);
            }
            return error.BinCreationFailed;
        }
        // The parser can return a bin AND set a recoverable error.
        if (err) |e| {
            std.log.warn("Bin created with warnings: {s}", .{e.message});
            c.g_error_free(e);
        }
