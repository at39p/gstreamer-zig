...
pub const AppSink = struct {
    ...
    /// Unrefs the element only if it has no parent. Once added to a bin or
    /// pipeline, the parent owns the element's reference (gst_bin_add sinks
    /// the floating ref), so unreffing here would over-release it.
