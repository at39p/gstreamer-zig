...
pub const Caps = struct {
    ...
    pub fn setField(self: Caps, name: [*:0]const u8, value: anytype) void {
        ...
        switch (T) {
            ...
            i64, c_long => {
                // Always pass 64 bits: the G_TYPE_INT64 collector reads a
                // gint64 vararg, and c_long is only 32-bit on some platforms.
                c.gst_structure_set(structure, name, c.G_TYPE_INT64, @as(i64, value), @as(?*anyopaque, null));
            },
            u32, c_uint => {
                c.gst_structure_set(structure, name, c.G_TYPE_UINT, @as(c_uint, value), @as(?*anyopaque, null));
            },
            u64 => {
                c.gst_structure_set(structure, name, c.G_TYPE_UINT64, @as(u64, value), @as(?*anyopaque, null));
            },
            f32 => {
                // C default argument promotion: the G_TYPE_FLOAT collector
                // reads a double from the varargs.
                c.gst_structure_set(structure, name, c.G_TYPE_FLOAT, @as(f64, value), @as(?*anyopaque, null));
            },
            f64 => {
